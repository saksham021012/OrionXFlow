...
export const extractFrameTask = task({
    ...
    run: async (payload: NodeExecutionPayload) => {
        return handleNodeExecution(payload, 'extractFrame', async (inputs: {
            videoUrl: string
            ...
            try {
                ...
                if (typeof inputs.timestamp === 'string' && inputs.timestamp.includes('%')) {
                    // Percentage-based timestamp
                    const percentage = parseFloat(inputs.timestamp.replace('%', ''))

                    // Get video duration from remote URL
                    const duration = await new Promise<number>((resolve, reject) => {
                        ffmpeg.ffprobe(inputs.videoUrl, (err, metadata) => {
                            if (err) reject(err)
                    ...
                    seekTime = inputs.timestamp.toString()
                }

                // Extract frame using FFmpeg directly from URL
                await new Promise<void>((resolve, reject) => {
                    ffmpeg(inputs.videoUrl)
                        .seekInput(seekTime)
                        .frames(1)
                        .output(outputPath)
                        .on('end', () => resolve())
