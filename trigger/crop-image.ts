...
export const cropImageTask = task({
    ...
    run: async (payload: NodeExecutionPayload) => {
        return handleNodeExecution(payload, 'cropImage', async (inputs: {
            imageUrl: string
            xPercent: number
            yPercent: number
            widthPercent: number
            ...
            try {
                // Download image
                const response = await fetch(inputs.imageUrl)
                ...
                const cropX = Math.floor((metadata.width * inputs.xPercent) / 100)
                const cropY = Math.floor((metadata.height * inputs.yPercent) / 100)
                const cropWidth = Math.floor((metadata.width * inputs.widthPercent) / 100)
                const cropHeight = Math.floor((metadata.height * inputs.heightPercent) / 100)
